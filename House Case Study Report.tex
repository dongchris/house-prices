...
\includegraphics[scale = 0.34]{plot2.png}
...
\centering
    \includegraphics[scale = 0.15]{plot3.png}\\[1.0 cm]
\end{flushleft}
\begin{flushleft}
...
lotarea & bsmtexposure   &  garagetype \\ neighborhood &  bsmtfinsf1 &    garagecars \\
  condition1  & bsmtfinsf2    & wooddecksf \\
   ...
   masvnrarea &  functional &       newtype \\  \hline 
...
\centering
    \vspace*{0.5 cm}
    \includegraphics[scale = .5]{leverage.png}\\[1.0 cm]	
...
\centering
    \vspace*{0.5 cm}
    \includegraphics[scale = .5]{influence.png}\\[1.0 cm]	

\begin{flushleft}

\subsection{Outliers}
An observation i is an outlier if the residual $e_{i}$ is large. To determine whether an observation is an outlier, we will use the function ols\_dffits\_plot from the library olsrr. It is similar to what we did with influential points. It categorizes an outlier if it has a DFFIT greater than the threshold of 0.28. Note that it is more strict than  our high influence threshold. 
...
We begin our valuation of Morty's house by only selecting the 25 variables that are present in our final model. We predict the log(saleprice) then take the exponential. Computing the confidence interval we get the following: 
...
\textbf{According to our model, Morty can sell his house for a maximum value of \$ 196833.60. He should consider improving overallqual, kitchenqual, masvnrarea to increase the value of his home before putting it on the market.}
