...
```{r}
exp(predict(endmodel, morty2, interval = "confidence", level = 0.95)) %>% knitr::kable()
```
...
index <- which(morty2!=mean_stat)
morty_stat[index]
mean_stat[index]

improve <- house4[index] %>% select(-saleprice, -soldminusbuilt) %>% 
  sapply(function(x) abs(cor(x, house4$saleprice))) %>% sort(decreasing = T)
```

```{r}
improve

for(i in 1:length(morty_stat)){
  print(i)
}
